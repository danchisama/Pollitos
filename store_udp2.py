...
#def send_data(bytes, addres):
#    return UDPClientSocket.sendto(bytes, addres)

def data_inserted(formato, datos):
    connection = mysql.connect()
    cursor = connection.cursor()
    cursor.execute(formato,tuple(datos))
    connection.commit()
    print "Fin"
...
while(True):
    ...
    #msgFromClient = binascii.hexlify(AckMessage)
    #msgFromClient = binascii.unhexlify(AckMessage)
    msgFromClient = AckMessage
    print "AckMessage: ", msgFromClient

    #bytesToSend = binascii.unhexlify(msgFromClient)
    #bytesToSend = binascii.hexlify(msgFromClient)
    ...
    #data_send = send_data(bytesToSend, serverAddressPort)
    #print "data_send: ", data_send


    #data_inserted(insertSQL, sqlValues)


    # Send to server using created UDP socket
    size = UDPClientSocket.sendto(bytesToSend, serverAddressPort)
    print "Len sendto: ", size
